...
# Compute the monthly average AQI
county_data['YearWeek'] = county_data['Date'].dt.to_period('M')  # create a new column with the year and month
...
# Create a scatter plot of the monthly average AQI
ax.scatter(weekly_avg_aqi.index, weekly_avg_aqi, color='blue', label='Monthly Average AQI')
...
ax.set_title(f'Monthly Average AQI for {county_of_interest} County')
ax.set_ylabel('AQI')
ax.set_xlabel('Date')
